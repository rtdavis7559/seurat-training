...
The output which parts of X are located in Y. For our purposes we want the rownames(training.object@meta.data) (X) that are located in our doublets.ids (Y). To do this, we will execute the following code:
...
doublet.metadata[doublet.rows]<-"doublets"
doublet.metadata[c(1:3,12:15)]
```
...
```{r doublet.character}
doublet.metadata<-as.character(doublet.metadata)
doublet.metadata[c(1:3,13:15)]
```
...
```{r add.doublet.meta.data message=FALSE}
training.object[["doublets"]]<-doublet.metadata
colnames(training.object@meta.data)
